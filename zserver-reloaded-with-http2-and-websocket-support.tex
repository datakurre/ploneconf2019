...
\begin{frame}{WebSockets}
  \textbf{WebSockets with custom PubSub -protocol}
  \begin{itemize}
    \item Autobahn Twisted WebSocket protcol
    \item ZMQ PubSub -socket using IPC-socket
    \item Connection upgradeable at all paths
    \item Connection subscribes to events at path
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Publish-Subscribe}
  \textbf{Client subscribes events}
  \footnotesize
  \begin{minted}{JSON}
...
\begin{frame}[fragile]{Guarded Events}
  \textbf{Server publishes event with guard}
  \footnotesize
  \begin{minted}{JSON}
{
  "guards": [{
...
\begin{frame}[fragile]{Guard Response}
  \textbf{Server responses guard check once for each subscriber}
  \footnotesize
  \begin{minted}{JSON}
{
  "allowedRolesAndUsers": {
...
\begin{frame}{State of Upstream}
  ...
  \textbf{Zope 5}
  \begin{itemize}
    \item Assumes ZServer to no longer exist
  \end{itemize}
\end{frame}
...
\begin{frame}{Discussion}
  ...
  \begin{itemize}
    ...
    \item QA (tests, documentation)
    \item Release
    \item Merge \texttt{wsgi=off}
  \end{itemize}
\end{frame}
...
\par
\href{https://datakurre.github.io/ploneconf19/zserver}{datakurre.github.io/ploneconf19/zserver}
\end{frame}
